...
class MSP_Dataset(torch.utils.data.Dataset):
    def __init__(self, df_speech, speech_dir, sr, norm_param, noise_dir_classes, snr):
    ...
    def mix_with_noise(self, wav):
        ...
        mixed = F.add_noise(wav.unsqueeze(0), noise_wav.unsqueeze(0), torch.tensor([self.snr]))
        return mixed.squeeze(0), self.snr, scene, noise_wav, selected_file
