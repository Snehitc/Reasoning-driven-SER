...
class MellowWrapper():
    ...
    def Custom_load_audio_into_tensor(self, custom_audio_tensor, audio_duration, current_sr, resampler, resample=True):
        r"""Loads audio file and returns raw audio."""
        # Randomly sample a segment of audio_duration from the clip or pad to match duration
        #audio_time_series, sample_rate = torchaudio.load(audio_path)
        audio_time_series, sample_rate = (custom_audio_tensor, current_sr)
    ...
    def Custom_preprocess_audio(self, custom_audio_tensors, current_sr, resample):
        r"""Load list of audio files and return raw audio"""
        resample_rate = self.args.data["sampling_rate"]
        if resample and resample_rate != current_sr:
            resampler = T.Resample(current_sr, resample_rate).to(f"cuda:{self.device}")
            
            
        audio_tensors = []
        for custom_audio_tensor in custom_audio_tensors:
            audio_tensor = self.Custom_load_audio_into_tensor(
                custom_audio_tensor, self.args.data["segment_seconds"], current_sr, resampler, resample)
    ...
    def _generate_batch(
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(entry_length), disable=True):
    ...
    def Custom_generate(self, examples, max_len, top_p, temperature, current_sr, stop_token='<|endoftext|>', audio_resample=True):
        r"""Produces text response for the given audio file and text prompts
        examples: (list<list>) List of examples. Each example is a list containing three entries [audio path 1, audio path 2, text prompt]
        max_len: (int) maximum length for text generation. Necessary to stop generation if LM gets "stuck" producing same token
        temperature: (float) top-p parameter for LM sampling
        ...
        """
        preds = []
        custom_audio_tensors1 = []
        #custom_audio_tensors2 = []
        text_prompts = []
        #batch_audio_tensors, common_prompt = examples
        for example in examples:
            ap1, tp = example
            custom_audio_tensors1.append(ap1)
            #custom_audio_tensors2.append(ap2)
            text_prompts.append(tp)
        
        audio1_embed = self.Custom_preprocess_audio(custom_audio_tensors1, current_sr, resample=audio_resample).squeeze(1)
        #audio2_embed = self.Custom_preprocess_audio(custom_audio_tensors2, current_sr, resample=audio_resample).squeeze(1)
        text_embed = self.preprocess_text(text_prompts)
        d = {
                "audio1": audio1_embed,
                "audio2": audio1_embed[:,:513], # keep it anything more than 512
                "input": text_embed,
            }
        prefix, _, _ = self.model.generate_prefix_inference(d)
        preds = self._generate_batch(embed=prefix, top_p=top_p, temperature=temperature, stop_token=stop_token, entry_length=max_len)
